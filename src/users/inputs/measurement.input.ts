...
import { BodyMeasurementsModel } from '../models';

@InputType({
    description: 'Модель измерений пользователя которые можно изменить',
})
export class BodyMeasurementInput extends PartialType(BodyMeasurementsModel) {
    ...
    chestCm?: string;
    @IsString({ message: 'Обхват талии должен быть строкой' })
    @IsOptional({ message: 'Обхват талии не обязателен' })
    @Field(() => String, {
        nullable: true,
    ...
    @IsEnum(ActivityLevel, { message: 'Некорректный уровень активности' })
    @Field(() => ActivityLevel, {
        nullable: true,
